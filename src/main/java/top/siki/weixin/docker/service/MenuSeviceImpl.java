...
public class MenuSeviceImpl implements MenuService {
    ...
    public int createMenu(Map<String, Object> menu,String accessToken) {
        ...
        if (null != jsonObject) {
            if (0 != jsonObject.get("errcode").getAsInt()) {
                ...
                log.error("创建菜单失败 errcode:{} errmsg:{}", jsonObject.get("errcode").getAsInt(), jsonObject.get("errmsg").getAsString());
                log.error("****"+jsonMenu+"****");
            }
        }
        return result;
    ...
    public int deleteMenu(String accessToken) {
        ...
        if (null != jsonObject) {
            if (0 != jsonObject.get("errcode").getAsInt()) {
                result = jsonObject.get("errcode").getAsInt();
                log.error("删除菜单失败 errcode:{} errmsg:{}", jsonObject.get("errcode").getAsInt(), jsonObject.get("errmsg").getAsString());
            }
        }
        return result;
    }
